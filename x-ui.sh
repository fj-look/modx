...
install() {
    bash <(curl -Ls https://jsd.cdn.zzko.cn/gh/fj-look/modx@main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://jsd.cdn.zzko.cn/gh/fj-look/modx@main/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
